...
type TCheckedFilterItem = string | number | Date
            ...
            if (checkedGroup[key].length === 3) {
              if (checkedGroup[key][0] === 'custom') {
                ...
                const tag = `${item!.title}: ${
                  item!.options!.find((option) => option.key === checkedGroup[key][0])!.label
                }`
                tags.push({ id: item!.id, selectedKey: key, tag, type: item!.type })
              }
            }
              ...
              return (
                <FilterDateRangeAccordion
                  ...
                  options={item.options.map(
                    (option) => ({ key: option.key, text: option.label }) as IChoiceGroupOption
                  )}
                  defaultSelectedKey={item.selectedKey}
                  start={item.start}
                  end={item.end}
            ...
            case EFilterItemType.Text:
              return (
                <FilterTextAccordion
                  ...
                  updateData={(textValue) => onTextCheckedChanged(item, textValue)}
                />
              )
            case EFilterItemType.Checkbox:
            default:
...
const FilterCheckboxAccordion: React.FunctionComponent<IFilterCheckboxAccordionProps> = (
  props: IFilterCheckboxAccordionProps
) => {
        ...
        <>
          ...
          <div style={{ maxHeight: 250, overflowY: 'auto', width: '100%', paddingBottom: 17, paddingTop: 12 }}>
            {options.map((checkbox) => {
              if (checkbox.visibility === EVisibility.hidden)
                return <React.Fragment key={`${id}-checkbox-${checkbox.key}`} />
              return (
                <Checkbox
...
const FilterRadioAccordion: React.FunctionComponent<IFilterRadioAccordionProps> = (
  props: IFilterRadioAccordionProps
) => {
  ...
  const checkedItem = React.useMemo(() => options.find((option) => option.key === selectedKey), [selectedKey, options])
            ...
            <div style={{ maxHeight: 250, overflowY: 'auto', width: '100%', paddingBottom: 17, paddingTop: 12 }}>
              <ChoiceGroup
                options={filteredOptions
                  .filter((option) => option.visibility === EVisibility.visible)
                  .map((option) => ({ key: option.key, text: option.label }) as IChoiceGroupOption)}
                required={true}
...
const FilterDateRangeAccordion: React.FunctionComponent<IFilterDateRangeAccordionProps> = (
  props: IFilterDateRangeAccordionProps
) => {
  ...
  }, [options, defaultSelectedKey, end, start])
                ...
                <Stack styles={pageStyles} style={{ marginLeft: 22 }}>
                  <DateRangePicker
                    ...
                    onChangeDate={(previousDate: Date, nextDate: Date) => {
                      ...
                      const from =
                        previousDate.getMonth() + 1 + '/' + previousDate.getDate() + '/' + previousDate.getFullYear()
                      setToDate(to)
                      setFromDate(from)
...
const FilterTextAccordion: React.FunctionComponent<IFilterTextAccordionProps> = (props: IFilterTextAccordionProps) => {
